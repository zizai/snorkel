...
class PatternMatchGenerator(object):
    """
...
class MatchTerms(PatternMatchGenerator):
    """
    Match over several term dictionaries
    """
    def __init__(self, name, terms, rvalue, search='sentence', window=1, attrib='words'):
    ...
    def lf(self):
        def f(c):
            ...
            return self.rvalue if len(matches) > 0 else 0

        params = "[{}|{}{}]".format(self.search, self.attrib, "" \
            if self.search in ['sentence', 'between'] \
            else "|window={}".format(self.window))

        args = ['LF_TERMS', self.name, params, 'TRUE' if self.rvalue == 1 else 'FALSE']
        f.__name__ = "_".join(args)
        return f


class MatchRegex(PatternMatchGenerator):
    """
    Match over a set of provided regular expressions
    """
    def __init__(self, name, rgxs, rvalue, search='sentence', window=1, attrib='words'):
    ...
    def lf(self):
        ...
        args = ['LF_REGEX', self.name, params, 'TRUE' if self.rvalue == 1 else 'FALSE']
        f.__name__ = "_".join(args)
        return f
