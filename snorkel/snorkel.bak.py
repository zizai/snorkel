...
class SpanLearner(PipelinedLearner):
    ...
    def train_lf_model(self, w0=5.0, **model_hyperparams):
        ...
        if type(self.gen_model) is MnLogReg:
            ...
            # print len(f_bags),len([x for x in itertools.chain.from_iterable(f_bags)])

            # marginals for multinomals are per spanset, so we need to flatten
            # matrices back to our original observed candidate set
            # candidate with no LF coverage are assumed 0.5 (random)

            for i, c in enumerate(self.training_set.training_candidates):
                if c not in cand_index:
                    # HACK fix this
                    print>> sys.stderr, "MISSING in bag", c
                    marginals.append(0.5)
                    continue

                j = cand_index[c]
                mention = c.get_attrib_span("words").replace(" ", "")
                classes = [x.replace(" ", "") for x in self.classes[j]]
                if mention not in classes:
                    ...
                    marginals.append(mn_marginals[j][k])
    ...
    def candidate_multinomials(self, candidates, split_chars=["/", "-"]):
        ...
        tokens = candidates[0].sentence["words"][idxs[0]:idxs[-1]+1]
        ...
        #         seq = seq[len(t):].strip()
        #
        # f_tokens = [x for x in itertools.chain.from_iterable([tokenize(t, split_chars) for t in f_tokens])]
    ...
    def lf_prob(self, include_neg=True, class_threshold=8192):
        ...
        # flush progress bar
        sys.stdout.write('Processing \r{:2.2f}% {}/{}\n'.format(100, i+1, len(self.span_bags)))
        sys.stdout.flush()
    ...
    def sample(self, marginals=None, num_samples=10, format="conll",
