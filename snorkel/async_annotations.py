    ...
    def update(self, session, candidate_set, key_set, expand_key_set, f=None):
        ...
        :param f: Can be either:
            ...
            * A list of functions, each of which maps from candidates to values; by default, the key_name
                is the function.__name__.  Ex: A list of labeling functions
        """
        global _global_candidates
        # Prepares arguments
        _global_candidates = get_ORM_instance(CandidateSet, session, candidate_set)
        ...
        # print "Generating %s Annotations for CandidateSet %s..." % (key_set.name, _global_candidates.name)
        pb = ProgressBar(len(_global_candidates))
               
        pool = multiprocessing.Pool()
        all_features = pool.map_async(default_featurize, range(len(_global_candidates)))
        pool.close()
        pool.join()
        all_features = all_features.get()
        print len(all_features)
        
        # Generates annotations for CandidateSet
        for i, (candidate, features) in enumerate(izip(_global_candidates, all_features)):
            pb.bar(i)
            # Each candidate have unique
            candidate_key_ids = set()
            ...
            for key_name in features:
                ...
                if key_id in candidate_key_ids: continue
                # Record new annotation
                candidate_key_ids.add(key_id)
                annotations.append({'candidate_id': candidate.id, 'key_id': key_id, 'value': 1})
...
_default_features = get_span_feats
_global_candidates = None
def default_featurize(i):
    return [key_name for key_name, value in _default_features(_global_candidates[i])]
